        ...
        if($request->ajax())
        {
            ...
            return DataTables::of($data)
                ->editColumn('status', function($row) {
                    $statusLabel = ''; // Initialize an empty string for the status label

                    // Check the status and assign the appropriate label
                    if ($row->status == 1) {
                        $statusLabel = '<span class="badge badge-warning">Pending</span>';
                    } else if ($row->status == 2) {
                        $statusLabel = '<span class="badge badge-info">In Progress</span>';
                    } else if ($row->status == 3) {
                        $statusLabel = '<span class="badge badge-success">Resolved</span>';
                    }

                    return $statusLabel; // Return the status label
                })
                ->addColumn('action', function ($data) {
                return '<a href="'.route('user.view_chat', 'id='.$data->id).'" class="btn btn-sm btn-primary mr-1"  data-id="'.$data->id.'"><i class="mdi mdi-eye"></i></a>';

                // <a href="'.route('user.edit_message', $data->id).'" class="btn btn-sm btn-primary" data-id="'.$data->id.'"><i class="mdi mdi-square-edit-outline"></i></a>
            })
