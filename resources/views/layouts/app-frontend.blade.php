...
<body class="{{ $body_class }}">
            ...
            <div class="container">
                        ...
                        {{-- <li class="drop">
                            ...
                            <ul>
                                @foreach (getCategories() as $category)
                                    <li>
                                        <a href="{{ route('products.category', ['id' => $category->id]) }}"><img
                                                src="{{ getArtIcon($category->name)['image'] }}" alt=""
                                                class="img-menu-icon-w-32">{{ $category->name }}</a>
                                    </li>
                                @endforeach
                            </ul>
                        </li> --}}
                        <li><a href="{{ route('product.singleproduct') }}">Somaheal</a></li>
                        <li><a href="{{ route('frontend.authenticity') }}">Authenticity</a></li>
                        <li><a href="{{ route('frontend.about') }}">About SHF</a></li>
                    </ul>
                </div>
                <div class="search-field">
                    <input type="search" placeholder="Search SHF" class="search-input" name="search" id="search">
                ...
                <div class="head-right">
                    <div class="menuBtn">
                        ...
                        @if (!Auth::user())
                            <a href="#sign-in" class="button blue popup-link1">Sign In</a>
                        @else
    ...
    <footer id="footer">
        ...
        <div class="subscribe-wrap">
            <div class="container">
                <div class="subscribe">
                    <h3>You have more questions?</h3>
                    <p class="txt-1 center">Contact us with your request directly using online contact form or send us
                        your
                        email address and we will make sure you get your response quickly
                    </p>
                    <form id="subscribe-form" class="subscribe-form">
                        @csrf
                        <div class="form-grp form-grp-footer-custom">
                            <input type="email" name="email" id="email" placeholder="your@email.com"
                                class="footer-field"><br>
        ...
        <div class="footer-wrap">
            <div class="container">
                <div class="footer-top">
                    ...
                    <p><img src="{{ $baseUrl }}img/copyright.svg" alt=""> SHF Laboratories
                    </p>
                </div>
                <div class="footer-bottom">
                    <a href="mailto:info@shf-science.com"><img src="{{ $baseUrl }}img/at.svg" alt="">
                        info@shf-science.com</a>
            ...
            <div class="popup-content">
                ...
                <div class="popup-right">
                    ...
                    <form action="" method="POST" id="register-form" autocomplete="off">
                        @csrf
                        <div class="form-grp">
                            <input class="form-field" type="text" id="first_name" name="first_name"
                                placeholder="First Name" />
                            <div class="error"></div>
                        </div>

                        <div class="form-grp">
                            <input class="form-field" type="text" id="last_name" name="last_name"
                        ...
                        <div class="form-grp">
                            <input class="form-field" type="phone_number" id="phone_number" name="phone_number"
                        ...
                        <div class="form-grp">
                            <input class="form-field" type="password" id="password-confirm"
                                name="password_confirmation"placeholder="Confirm Password" />
                        </div>
                        <button class="button blue" type="submit">Sign Up</button>
                    </form>
                    <a href="#forgot-pass" class="popup-link1 linkGoto">Forgot password?</a>

                    <p class="bottom-line">Already a member?
                        <a href="#sign-in" class="popup-link1 linkGoto">Sign In.</a>
                    </p>
                </div>
            </div>
        </div>
        ...
        <div id="email-verification" class="white-popup mfp-with-anim mfp-hide onboard-journey-popup">
            <div class="popup-content">
                ...
                <div class="popup-right check-email-right">
                    ...
                    <p class="bottom-line">We sent a confirmation link to <br /><span
                            id="email-address-placeholder"></span>
                    </p>
                    ...
                    <a href="#sign-in" class="popup-link1 linkGoto center"><img
                            src="{{ $baseUrl }}img/back-arrow.svg" alt=""> Go back to Sign In</a>
                    <p class="bottom-line">Not a member yet?
                        <a href="#sign-up" class="popup-link1 linkGoto">Sign up</a>
                    </p>
                </div>
            </div>
        </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
                        ...
                        if (result.status == true) {
                            ...
                            if (result.message) {
                                ...
                                $.each(result.errors, function(key) {
                                    ...
                                    if (!key.includes("[]"))
                                        $('#' + key).closest('.form-grp').find('.error')
                ...
                $.ajax({
                    ...
                    error: function(response) {
                        ...
                        $.each(response.responseJSON.errors, function(key) {
                            if (first_input == "") first_input = key;
                            if (!key.includes("[]"))
                                $('#' + key).closest('.form-grp').find('.error').html(
                        ...
                        if (data.hasOwnProperty('filteredProducts')) {
                            ...
                            for (let category in data.filteredProducts) {
                                if (data.filteredProducts[category].length > 0) {
                                    resultsFound = true;
                                    $('#search-results-list').append('<li><strong>' + category +
                                        '</strong></li>');
                                    $.each(data.filteredProducts[category], function(index, product) {
                                        $('#search-results-list').append(
                                            '<li style="margin-left:10px;"><a href="{{ url('product/detail') }}/' +
                    ...
                    success: function(result) {
                        ...
                        if (result.status == false) {
                            ...
                            $.each(result.errors, function(key) {
                                if (first_input == "") first_input = key;
                                if (!key.includes("[]"))
                                    $('#subscribe-form #' + key).closest('.form-grp')
                                    .find('.error').html(result.errors[key]);
                            });
                        }
                    },
                    error: function(response) {
            ...
            function setupFormValidation(formSelector) {
                $(formSelector).on('keyup change', 'input, textarea, select, option', function(event) {
                    var $input = $(this);
                    if ($.trim($input.val())) {
                        $input.removeClass('error');
                        $input.closest('.form-grp').find('.error').html('');
                    }
                });
            }

            setupFormValidation('#subscribe-form');
            setupFormValidation('#login-form');
            setupFormValidation('#register-form');
            setupFormValidation('#forgot-password-form');
            // $('#subscribe-form').on('keyup change','input, textarea, select, option',function(event) {
            //     if($.trim($(this).val()) && $(this).val().length >0){
            //         $(this).removeClass('error');
            //         $(this).closest('.form-grp').find('.error').html('');
            //     }
            // });

            // $('#login-form').on('keyup change','input, textarea, select, option',function(event) {
            //     if($.trim($(this).val()) && $(this).val().length >0){
            //         $(this).removeClass('error');
            //         $(this).closest('.form-grp').find('.error').html('');
            //     }
